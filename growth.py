...
if uploaded_file:
    for file in uploaded_file:
        ...
        if file_ext == ".csv":
            ...
            st.error("File format not supported. Please upload a CSV or Excel file: {file_ext}") 
            continue
        ...
        if st.checkbox(f"clean data for {file.name}"):
            ...
            with col1:
                ...
                if st.button(f"Remove duplicates from the file: {file.name}"):
                    ...
                    columns = df.multiselect(f"Choose Columns for {file.name}", df.columns, default=df.columns)
                    df= df[columns]
